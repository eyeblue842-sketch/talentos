...
describe('role routing', () => {
  test('maps canonical roles to their home routes', () => {
    ...
    expect(getHomeRouteForRole('RECRUITER')).toBe('/recruiter/home');
    expect(getHomeRouteForRole('ADMIN')).toBe('/admin');
    expect(getHomeRouteForRole('SUPER_ADMIN')).toBe('/admin');
    expect(getHomeRouteForRole('HIRING_MANAGER')).toBe('/recruiter/home');
  ...
  test('keeps candidate-safe destinations and rejects recruiter access to candidate pages', () => {
    ...
    expect(resolvePostAuthRoute('RECRUITER', '/candidate/applications')).toBe('/recruiter/home');
  });
  ...
  test('keeps recruiter-safe destinations and normalizes auth or root entry points to home', () => {
    ...
    expect(resolvePostAuthRoute('RECRUITER', '/auth')).toBe('/recruiter/home');
  ...
  test('RECRUITER_ADMIN is the recruiter application owner: lands on the recruiter workspace, admin console reachable as an extra surface', () => {
    expect(getHomeRouteForRole('RECRUITER_ADMIN')).toBe('/recruiter/home');
    expect(resolvePostAuthRoute('RECRUITER_ADMIN', '/recruiter/home')).toBe('/recruiter/home');
    ...
    expect(resolvePostAuthRoute('RECRUITER_ADMIN', '/candidate/applications')).toBe('/recruiter/home');
  });
