...
async function createBackendApiContext(email: string, password: string) {
  const authContext = await playwrightRequest.newContext();
  const loginResponse = await authContext.post('http://127.0.0.1:5001/api/auth/login', {
    data: { email, password },
  });
...
test.describe.serial('Careeriz Messaging Phase 1 browser E2E', () => {
  test('candidate to recruiter flow clears unread and keeps duplicate conversation creation idempotent', async ({ browser }) => {
    ...
    const candidateAApi = await createBackendApiContext(seed.candidateA.email, seed.password);
  ...
  test('candidate to candidate flow clears unread and block disables further messaging', async ({ browser }) => {
    ...
    try {
      ...
      const candidateACardForBlock = candidateB.getByTestId(`network-user-${seed.candidateA.userId}`);
      await expect(candidateACardForBlock).toBeVisible({ timeout: 30000 });
      const candidateABlockButton = candidateACardForBlock.getByTestId(`network-block-${seed.candidateA.userId}`);
      ...
      await expect(candidateACardForBlock.getByText(/blocked/i)).toBeVisible({ timeout: 30000 });
  ...
  test('recruiter to candidate flow clears unread and disconnect preserves history while disabling send', async ({ browser }) => {
    ...
    try {
      ...
      await candidateA.goto('/candidate/network', { waitUntil: 'domcontentloaded' });
      await expectNetworkPage(candidateA, 'candidate');
      const recruiterCardForDisconnect = candidateA.getByTestId(`network-user-${seed.recruiterA.userId}`);
      await recruiterCardForDisconnect.getByTestId(`network-remove-${seed.recruiterA.userId}`).click();
      await expect(recruiterCardForDisconnect.getByTestId(`network-message-${seed.recruiterA.userId}`)).toHaveCount(0, { timeout: 30000 });
  ...
  test('polling updates active conversations, unread badges, ordering, and avoids active polling when no conversation is open', async ({ browser }) => {
    ...
    const candidateA = await browser.newPage();
    const candidateAApi = await createBackendApiContext(seed.candidateA.email, seed.password);
    const candidateBApi = await createBackendApiContext(seed.candidateB.email, seed.password);
    const recruiterAApi = await createBackendApiContext(seed.recruiterA.email, seed.password);
