...
test('query compiler enforces MUST/SHOULD/MUST_NOT semantics without query_string', () => {
  ...
  assert.equal(compiled.searchRequest.query.bool.must.length, 2);
  assert.equal(compiled.searchRequest.query.bool.should.length, 1);
  assert.equal(compiled.searchRequest.query.bool.must_not.length, 1);
...
test('query compiler requires one optional match when only SHOULD keywords are present', () => {
  const compiled = compileResumeSearchV2Query({
    keywords: [
...
test('document builder strips contact data, omits resumeText, and derives global visibility by default for recruiter-visible profiles', () => {
  const candidate = {
    id: 'candidate-1',
